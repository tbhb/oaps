...
# ruff: noqa: D415, A002, ARG001, BLE001, PLR0911, PLR0912, S108
...
def _test(
    *,
    ...
    if rule_id:
        ...
        if not rules:
            exit_with_error(f"Rule '{rule_id}' not found", ExitCode.NOT_FOUND)
    ...
    if model_class is None:
        exit_with_error(f"No input model for event type '{event}'", ExitCode.IO_ERROR)
    ...
    if input_json:
        try:
            ...
            exit_with_error(f"Parsing input JSON: {e}", ExitCode.IO_ERROR)
    else:
        ...
        try:
            ...
            exit_with_error(f"Creating minimal input: {e}", ExitCode.IO_ERROR)
