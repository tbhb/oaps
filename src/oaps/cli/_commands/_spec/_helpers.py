...
def parse_qualified_id(qualified_id: str, console: Console) -> tuple[str, str]:
    ...
    Raises:
        SystemExit: With EXIT_VALIDATION_ERROR if format is invalid.
    """
    from ._errors import EXIT_VALIDATION_ERROR

    parts = qualified_id.split(":", 1)
    if len(parts) != 2 or not parts[0] or not parts[1]:
        console.print(f"[red]Error:[/red] Invalid qualified ID format: {qualified_id}")
...
def print_spec_result(data: SpecData, format_: OutputFormat) -> None:
    ...
    """
    from ._errors import EXIT_SUCCESS
