...
def _create(
    ...
    try:
        ...
        exit_with_error(
            f"Invalid type '{type}'. Valid: {valid_types}",
...
def _list(
    ...
    if type:
        try:
            ...
            exit_with_error(
                f"Invalid type '{type}'. Valid: {valid}",
                ExitCode.VALIDATION_ERROR,
                console=console,
            )
...
def _show(idea_id: str, /) -> None:
    ...
    if path is None:
        exit_with_error(
            f"Idea not found: {idea_id}", ExitCode.NOT_FOUND, console=console
        )
...
def _link(idea_id: str, related_id: str, /) -> None:
    ...
    if path2 is None:
        exit_with_error(
            f"Related idea not found: {related_id}", ExitCode.NOT_FOUND, console=console
        )
...
def _tag_describe(name: str, /) -> None:
    ...
    if name not in all_tags:
        ...
        exit_with_error(
            f"Tag '{name}' not found in configuration.",
            ExitCode.NOT_FOUND,
            console=console,
        )
