    ...
    *,
    ...
    if worktree_root is None and (config.worktree_gitignore or config.oaps_gitignore):
        ...
        try:
            ...
            # If we can't find worktree, skip file-based patterns
            worktree_root = None
...
def matches_any(pathspec: PathSpec, path: str | Path) -> bool:
    """Check if a path matches the ignore patterns.

    Args:
        pathspec: The PathSpec to match against.
        path: The path to check. Can be absolute or relative.

    Returns:
        True if the path matches any pattern in the pathspec.
    """
