...
class PythonAction:
    ...
    def run(
        ...
        # Parse entrypoint as "module.path:function_name"
        if ":" not in entrypoint:
            logger.warning(
                "PythonAction: invalid entrypoint format (expected module:function)",
        ...
        # Determine timeout (convert ms to seconds)
        timeout_ms = config.timeout_ms or DEFAULT_TIMEOUT_MS
        ...
        try:
            ...
            # Get the function - returns Any since module attributes are untyped
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    result: object = future.result(timeout=timeout_seconds)
            ...
            if isinstance(result, dict):
                return_value = dict(result)  # pyright: ignore[reportUnknownArgumentType]
...
class TransformAction:
    ...
    def _run_python_transform(
        ...
        if ":" not in entrypoint:
            logger.warning(
                "TransformAction: invalid entrypoint format (expected module:function)",
                entrypoint=entrypoint,
            )
            return
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                transform_data: object = result_dict.get("transform_input")
